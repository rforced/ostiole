...
func TestTokenRoundTrip(t *testing.T) {
	...
	tok, secret, err := tk.Create("monitoring", RoleViewer, 0, "admin")
	if err != nil {
		t.Fatal(err)
	}
	if tok.Hash != "" {
...
func TestAuthenticateRejects(t *testing.T) {
	t.Parallel()
	tk := tokens(t)
	_, secret, err := tk.Create("real", RoleAdmin, 0, "")
	if err != nil {
		t.Fatal(err)
	}
...
func TestTokenExpiry(t *testing.T) {
	...
	_, secret, err := tk.Create("short", RoleViewer, 24*time.Hour, "")
	if err != nil {
		t.Fatal(err)
	}
...
func TestTokenNamesAreUniqueAndChecked(t *testing.T) {
	t.Parallel()
	tk := tokens(t)
	if _, _, err := tk.Create("taken", RoleViewer, 0, ""); err != nil {
		t.Fatal(err)
	}
	if _, _, err := tk.Create("TAKEN", RoleViewer, 0, ""); err == nil {
		t.Error("a duplicate name was accepted")
	}
	if _, _, err := tk.Create("", RoleViewer, 0, ""); err == nil {
		t.Error("an empty name was accepted")
	}
	if _, _, err := tk.Create("fine", "wizard", 0, ""); err == nil {
...
func TestDeleteTokenByNameOrID(t *testing.T) {
	t.Parallel()
	tk := tokens(t)
	byName, _, _ := tk.Create("by-name", RoleViewer, 0, "")
	byID, _, _ := tk.Create("by-id", RoleViewer, 0, "")
...
func TestTokensFollowTheFile(t *testing.T) {
	...
	_, secret, err := first.Create("shared", RoleOperator, 0, "")
	if err != nil {
		t.Fatal(err)
	}
