...
func newAliasesCmd(g *globals) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `Blocklists, country address ranges and the prefixes behind AS numbers are
fetched rather than typed, and cached on this router so a reboot without a
working line still blocks what it blocked yesterday. The daemon refreshes
them on a schedule; this shows what it has and refreshes on demand.`,
		Args: cobra.NoArgs,
