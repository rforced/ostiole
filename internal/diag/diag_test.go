...
func TestJournalArgsTakeTheNewestFirst(t *testing.T) {
	...
	want := []string{
		"--output=json", "--no-pager", "--all", "--reverse", "--lines=50",
		"--unit=ostiole.service", "--since=-1h", "--priority=4",
	}
	if !slices.Equal(args, want) {
		t.Errorf("args = %q, want %q", args, want)
	}
	// A line count that makes no sense falls back rather than reading the
	// whole journal.
	args, err = journalArgs(JournalOptions{Lines: MaxJournalLines + 1})
	if err != nil {
		t.Fatal(err)
	}
	if !slices.Contains(args, "--lines=200") {
