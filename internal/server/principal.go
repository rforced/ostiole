...
// authenticate resolves the caller from the Authorization header or the
// session cookie, in that order.
func (a *api) authenticate(r *http.Request) (Principal, bool) {
	if raw := bearer(r); raw != "" && a.tokens != nil {
		tok, err := a.tokens.Authenticate(raw)
		if err != nil {
			return Principal{}, false
		}
		return Principal{Name: tok.Name, Role: tok.Role, Token: true}, true
	}
	if a.auth == nil {
		return Principal{}, false
	}
