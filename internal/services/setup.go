...
type SetupOptions struct {
	...
	// Proxy writes the reverse proxy's user and unit.
	Proxy bool
...
// setupProxy creates the account the sidecar runs as and writes its unit.
// Nothing is started: the proxy comes up on the apply that gives it a
// site or a route to serve.
func setupProxy(ctx context.Context, run Runner, o SetupOptions, unitDir string, log *slog.Logger) error {
	bin := o.ProxyBinary
	if bin == "" {
		bin = lookPath(ProxyBinaryName)
	}
	if bin == "" {
		log.Info("no ostiole-proxy on this router; the reverse proxy is not set up")
		return nil
	}
	// A router that has never had a sysusers drop-in has no directory for
	// one, and systemd does not create it on demand.
	const sysusers = "/etc/sysusers.d/ostiole-proxy.conf"
	if err := os.MkdirAll(filepath.Dir(sysusers), 0o755); err != nil { //nolint:gosec // systemd reads this
		return err
	}
	if err := writeFile(sysusers, fmt.Sprintf("u %s - \"Ostiole reverse proxy\" -\n", ProxyUser)); err != nil {
		return err
	}
	if out, err := run.Run(ctx, "systemd-sysusers", sysusers); err != nil {
		return fmt.Errorf("create the %s account: %w: %s", ProxyUser, err, strings.TrimSpace(string(out)))
	}
	dir := ProxyDir(o.ConfigDir)
	if err := os.MkdirAll(dir, 0o750); err != nil {
		return err
	}
	// The proxy runs unprivileged under a configuration directory that
	// is root's alone, and has to get through it to reach its own.
	if gid, err := proxyGroup(); err == nil && gid >= 0 {
		if err := grantTraversal(filepath.Dir(dir), gid); err != nil {
			return err
		}
	...
	log.Info("reverse proxy ready", "unit", ProxyUnit, "binary", bin)
	return nil
}

// grantTraversal lets the proxy's group through a directory that is
// root's: execute without read, so what is inside stays root's business.
// The owner is left alone, which is what lets a test do this as itself.
func grantTraversal(path string, gid int) error {
