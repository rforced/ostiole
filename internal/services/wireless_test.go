...
// with5G is a copy of the card with its 5 GHz band altered; the tests share
...
func TestWirelessPreflightRefusals(t *testing.T) {
	...
	t.Run("wider than the card goes", func(t *testing.T) {
		...
		files := render(t, w, cfg)
		w.Probe = func(context.Context, string) (*wireless.Phy, error) {
			return with5G(phy, func(b *wireless.BandInfo) { b.MaxWidth = 80 }), nil
		}
		err := w.Preflight(t.Context(), files)
	...
	t.Run("a standard the card has not got", func(t *testing.T) {
		...
		files := render(t, w, oneNetwork(t))
		w.Probe = func(context.Context, string) (*wireless.Phy, error) {
			return with5G(phy, func(b *wireless.BandInfo) { b.HE = false }), nil
		}
		err := w.Preflight(t.Context(), files)
...
// scanned, so the preflight scans first, the way the unit does, and judges
// the card on what it learned. A card the kernel governs means its no-IR
// and gets no scan.
func TestWirelessPreflightScansAFreshIntelCard(t *testing.T) {
	t.Parallel()
	learned := ax210(t)
	unlearned := func(b *wireless.BandInfo) {
		chans := append([]wireless.Channel(nil), b.Channels...)
		for i := range chans {
			chans[i].NoIR = true
		}
		b.Channels = chans
	}
	fresh := with5G(learned, unlearned)
	w, cmd, _ := newWireless(t, fresh)
	scanned := func() bool {
		for _, c := range cmd.calls {
			if len(c) == 4 && c[0] == "iw" && c[3] == "scan" {
				return true
			}
		}
		return false
	...
	files, err := w.Render(oneNetwork(t))
	if err != nil {
		t.Fatal(err)
	}
	if err := w.Preflight(t.Context(), files); err != nil {
		t.Fatalf("a fresh card was refused: %v", err)
	}
	if !scanned() {
		t.Errorf("the card was never scanned: %v", cmd.calls)
	}

	governed := with5G(learned, unlearned)
	governed.Driver = "ath11k"
	w2, cmd2, _ := newWireless(t, governed)
	err = w2.Preflight(t.Context(), files)
	if err == nil || !strings.Contains(err.Error(), "may not transmit on 5g") {
		t.Errorf("err = %v", err)
	}
	for _, c := range cmd2.calls {
		if strings.Contains(strings.Join(c, " "), "scan") {
			t.Errorf("scanned a card the kernel governs: %v", c)
		}
	}
}

// journalCmd answers journalctl with a canned log and everything else
// with nothing.
type journalCmd struct{ out string }

func (j journalCmd) Run(_ context.Context, name string, _ ...string) ([]byte, error) {
	if name == "journalctl" {
...
// hostapd's last complaint is the reason a radio is not on air; the event
// lines after it (AP-DISABLED, the state change) are not. With no
// complaint the last line is all there is to show.
func TestWirelessLastLogPicksTheComplaint(t *testing.T) {
	t.Parallel()
	w := &Wireless{Cmd: journalCmd{out: "ap0: interface state UNINITIALIZED->COUNTRY_UPDATE\n" +
		"ap0: Could not set channel for kernel driver\n" +
		"ap0: AP-DISABLED\n" +
		"ap0: interface state COUNTRY_UPDATE->DISABLED\n\n"}}
	if got := w.LastLog(context.Background(), "wlp3s0"); got != "ap0: Could not set channel for kernel driver" {
		t.Errorf("LastLog = %q", got)
	}
	w = &Wireless{Cmd: journalCmd{out: "ap0: AP-STA-DISCONNECTED 02:1a:2b:3c:4d:5e\nap0: AP-DISABLED\n"}}
	if got := w.LastLog(context.Background(), "wlp3s0"); got != "ap0: AP-DISABLED" {
		t.Errorf("LastLog without a complaint = %q", got)
	}
	if got := (&Wireless{Cmd: journalCmd{}}).LastLog(context.Background(), "wlp3s0"); got != "" {
		t.Errorf("LastLog of an empty journal = %q", got)
	}
}
