...
// SetSelfCertificate points the proxy at the pair the web UI serves, when
// it is not the default one under the configuration directory.
func (b *Bundle) SetSelfCertificate(cert, key string) {
	for _, back := range b.backends {
		if p, ok := back.(*Proxy); ok {
			p.SelfCert, p.SelfKey = cert, key
		}
	}
}
