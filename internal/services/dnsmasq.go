...
func (d *Dnsmasq) render(cfg *model.Config) (conf, hosts string, err error) {
	...
	if svc.DNS.Enabled {
		...
		if !svc.DNS.Rebind.Off {
			...
			// checks the answers from every server= it holds, unbound's
			// included.
			b.WriteString("stop-dns-rebind\nrebind-localhost-ok\n")
			if allowed := cfg.RebindAllowed(); len(allowed) > 0 {
				fmt.Fprintf(&b, "rebind-domain-ok=/%s/\n", strings.Join(allowed, "/"))
			}
		}
	...
	if svc.DHCP.Enabled {
		...
		// Who asked for an address, and when, is the most revealing thing
		// this daemon can write down, so the quiet options are the
		// default and only info turns them off. log-queries is never
		// written at any level.
		switch cfg.System.Logging.EffectiveLevel() {
		case model.LogError, model.LogWarning:
			b.WriteString("quiet-dhcp\nquiet-dhcp6\nquiet-ra\n")
		case model.LogDebug:
	...
	local := model.NormalizeDomain(svc.DNS.Domain)
	overrides := append([]model.HostOverride(nil), svc.DNS.HostOverrides...)
	sort.Slice(overrides, func(i, j int) bool { return overrides[i].FQDN(local) < overrides[j].FQDN(local) })
	for _, o := range overrides {
		// A bare label is answered under the local domain too, by
		// expand-hosts; a name with a dot in it is never expanded, so a
		// foreign domain is written in full. The primary comes first and
		// gets the PTR.
		names := make([]string, 0, 1+len(o.Aliases))
		for _, label := range append([]string{o.Hostname}, o.Aliases...) {
			if o.Local(local) {
				names = append(names, label)
			} else {
				names = append(names, label+"."+model.NormalizeDomain(o.Domain))
			}
		}
		fmt.Fprintf(&h, "%s %s\n", o.IP, strings.Join(names, " "))
	}
	for _, sh := range SystemHosts(cfg) {
		fmt.Fprintf(&h, "%s %s\n", sh.IP, sh.Hostname)
	}
...
// answer, in the order the hosts file carries them. An IPv6 host part like
// ::20 is only meaningful next to a prefix, so it is left out.
func SystemHosts(cfg *model.Config) []SystemHost {
	out := []SystemHost{}
	if cfg == nil {
		return out
	}
