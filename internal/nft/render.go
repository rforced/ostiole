...
// QueryLogGroup is the nflog group the resolver's answers are copied to
// while the query log is on; the daemon's query log listener reads it.
const QueryLogGroup = 2
...
func (r *renderer) serviceRules() {
	...
	if svc.DHCP.Enabled {
		...
		var v6 []string
		for _, sc := range r.cfg.ActiveDHCPv6() {
			if sc.Mode != model.RASLAAC {
...
// acmeRules let a CA reach the http-01 solver. The listener is only bound
// while a challenge is pending, but the rule stands for as long as a
// certificate is answering challenges: an apply in the middle of an order
// would otherwise shut the door on it.
func (r *renderer) acmeRules() {
	// With the proxy up, port 80 is its own and the challenge reaches the
	// solver through it, so this rule would only open a port nothing binds.
	if !r.cfg.HTTP01Certificates() || r.cfg.ProxyEnabled() {
		return
	}
	var ifs []string
	for _, z := range r.cfg.Zones {
...
// queryLogRule copies the resolver's answers to the daemon. Only what this
// router answers itself: a reply forwarded for a client that asked somebody
// else goes through forward and is not its answer.
func (r *renderer) queryLogRule() {
	dns := r.cfg.Services.DNS
	if !dns.Enabled || !dns.QueryLog.Enabled {
		return
	}
	ifs := DNSInterfaces(r.cfg)
	if len(ifs) == 0 {
	...
	r.line(fmt.Sprintf(`oifname %s udp sport 53 counter log group %d comment "dns-log"`,
...
func (r *renderer) defaultDrop(chain string) {
	logging := fmt.Sprintf(`counter log prefix "ostiole:c:%s:drop: " group %d comment "default-drop"`, chain, LogGroup)
	...
	r.line(fmt.Sprintf(`iifname %s counter log prefix "ostiole:c:%s:drop: " group %d drop comment "default-drop:logged"`,
...
func (r *renderer) blockedSources(chain string) {
	...
	emit := func(ifs []string, kind, what, set4, set6 string) {
		...
		for _, fam := range []struct {
			...
			if len(loud) > 0 {
				r.line(fmt.Sprintf(`iifname %s %s saddr @%s counter log prefix "ostiole:c:%s:%s: " group %d drop comment "%s"`,
...
func (r *renderer) zoneChains() {
	for _, z := range r.cfg.Zones {
		r.block("chain zone_"+z.Name, func() {
			...
			if z.LogDrops {
				key = "zone-default"
				r.line(fmt.Sprintf(`counter log prefix "ostiole:z:%s:drop: " group %d drop comment "zone-default"`, z.Name, LogGroup))
			} else {
...
// proxyRules open what the proxy listens on, at the tail of the zone's
// chain: behind the zone's flood and scan defence, and behind the
// operator's rules, so a drop written above this one narrows it.
func (r *renderer) proxyRules(z model.Zone) {
	if !r.cfg.ProxyEnabled() || !slices.Contains(r.cfg.Services.Proxy.ProxyZones(r.cfg), z.Name) {
		return
	}
	if len(r.cfg.ZoneInterfaces(z.Name)) == 0 {
		return
	}
	tcp, udp := r.cfg.ProxyPorts()
	for _, l := range []struct {
		proto   model.Protocol
		ports   []uint16
		comment string
	}{{model.ProtocolTCP, tcp, "service:proxy"}, {model.ProtocolUDP, udp, "service:proxy-udp"}} {
		if len(l.ports) == 0 {
			continue
		}
		ports := make([]string, 0, len(l.ports))
		for _, p := range l.ports {
			ports = append(ports, fmt.Sprint(p))
		}
		r.line(fmt.Sprintf(`fib daddr type local %s dport %s counter accept comment "%s"`,
			l.proto, setOrSingle(ports), l.comment))
		r.sys(SystemRule{
...
func (r *renderer) rule(rule *model.Rule) {
	...
	if rule.Log {
		// The action goes in the prefix because this is the only place it is
		// known for certain: the log statement and the verdict are the same
		// nft rule. Reading it back out of the config when the entry is
		// served would answer for the rule as it stands now, not as it stood
		// when the packet arrived.
		verdict = append(verdict,
			fmt.Sprintf("log prefix \"ostiole:r:%s:%s: \" group %d", rule.ID, rule.Action, LogGroup))
	}
