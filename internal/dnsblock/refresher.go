...
type Refresher struct {
	...
	OnTick func()
	// Installed, when set, is called with the options a newly installed
	// merge was rendered from. The query log rebuilds its index there, so
	// a list that has just changed is attributed from the names the
	// resolver is now refusing rather than the ones it was.
	Installed func(o Options)
}
