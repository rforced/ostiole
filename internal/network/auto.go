...
// Renew implements Renewer. Without networkd there is no lease to renew.
func (a *Auto) Renew(ctx context.Context, link string, release bool) error {
	if !a.Active(ctx) {
		return ErrNotRunning
	}
	return a.Networkd.Renew(ctx, link, release)
}
