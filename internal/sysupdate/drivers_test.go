...
func TestDNFUpgradeArgv(t *testing.T) {
	...
	if strings.Join(all, " ") != "dnf -y upgrade" {
		t.Errorf("all = %v", all)
	}
...
func TestDNFRebootRequired(t *testing.T) {
	...
	d := dnf{}
	run := &fakeRunner{
		out:  map[string]string{"dnf needs-restarting -r": "Core libraries or services have been updated since boot-up:\n  * systemd\n\nReboot is required to fully utilize these updates."},
	...
	clean := &fakeRunner{out: map[string]string{"dnf needs-restarting -r": "No core libraries or services have been updated since boot-up."}}
