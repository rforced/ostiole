...
func TestParsePhyReadsTheAX210(t *testing.T) {
	...
	// This card verifies beacons as a client and cannot sign its own, so
	// beacon protection stays off on it.
	if !p.Features["BEACON_PROTECTION_CLIENT"] {
		t.Error("the extended features were not read")
	}
	if p.BeaconProtection() {
		t.Error("the client-side flag was taken for the AP-side one")
	}
}

// Beacon protection goes on where the card can sign and the network has
// protected management frames, and nowhere else.
func TestRenderProtectsBeaconsWhereTheCardCan(t *testing.T) {
	t.Parallel()
	phy := ax210(t)
	phy.Features["BEACON_PROTECTION"] = true
	...
	for _, tc := range []struct {
		...
		{model.SecurityWPA3, true},
		{model.SecurityMixed, true},
		// WPA2 offers management frame protection rather than requiring
		// it, which is still enough for hostapd to sign the beacons.
		{model.SecurityWPA2, true},
		{model.SecurityOpen, false},
	} {
		nets := []model.Interface{network("ap0", "ostiole", tc.sec, "correct horse battery")}
		got := strings.Contains(Render("US", r, nets, &phy, nil, model.LogWarning), "beacon_prot=1")
		if got != tc.want {
			t.Errorf("%s beacon_prot = %v, want %v", tc.sec, got, tc.want)
		}
	}

	// And not at all on a card that cannot sign.
	plain := ax210(t)
	nets := []model.Interface{network("ap0", "ostiole", model.SecurityWPA3, "correct horse battery")}
	if strings.Contains(Render("US", r, nets, &plain, nil, model.LogWarning), "beacon_prot") {
		t.Error("beacon_prot rendered for a card that does not report it")
	}
}
...
func TestRenderGolden(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := Render("US", tc.radio, tc.nets, &phy, tc.master, model.LogWarning) +
...
func TestRenderWithoutAPhyLeavesOutCapabilities(t *testing.T) {
	t.Parallel()
	r := model.Radio{Name: "wlp3s0", Enabled: true, Band: model.Band5G, Channel: 36, Width: 80, Standard: model.StandardAX}
	got := Render("GB", r, []model.Interface{network("ap0", "ostiole", model.SecurityMixed, "correct horse battery")}, nil, nil, model.LogWarning)
...
// The fixture is a real dump from an AX210 serving two clients, taken on
// 2026-09-20: a laptop on VHT and a phone that had just joined. The
// field order is iw's, not a tidy one.
func TestParseStations(t *testing.T) {
	...
	want := Station{
		...
		ConnectedSeconds: 1052, InactiveMS: 15984,
		RxBytes: 4294527, TxBytes: 374971933, RxPackets: 37496, TxPackets: 85555,
		SignalDBm:  -45,
		TxBitrate:  "866.7 MBit/s VHT-MCS 9 80MHz short GI VHT-NSS 2",
		RxBitrate:  "866.7 MBit/s VHT-MCS 9 80MHz short GI VHT-NSS 2",
		Authorized: true,
	}
	if first != want {
		t.Errorf("station = %+v\nwant     %+v", first, want)
	...
	second := got[1]
	if !second.Authorized || second.SignalDBm != -55 || second.ConnectedSeconds != 82 || second.TxBitrate != "26.0 MBit/s MCS 3" {
		t.Errorf("second station = %+v", second)
	}
}
