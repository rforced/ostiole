...
<template>
  <div class="space-y-4">
    <div class="flex flex-wrap items-center gap-3">
      <RefreshButton :busy="live.busy.value" :updated-at="live.updatedAt.value" @click="live.run" />
    ...
    <p v-if="live.error.value" class="text-sm text-amber-700 dark:text-amber-400">
      {{ live.error.value }}
    </p>
    <p
      v-else-if="report && !report.available"
      class="text-sm text-amber-700 dark:text-amber-400"
      data-testid="shaping-unavailable"
    >
      {{ report.reason }}
    </p>

    <p v-if="report && !interfaces.length" class="text-sm text-neutral-500">
    ...
    <section
      ...
      :key="iface.name"
      class="card"
      :aria-labelledby="`live-${iface.name}`"
    >
      <h2 :id="`live-${iface.name}`" class="card-title">
        <span class="font-mono">{{ iface.name }}</span>
        <span v-if="iface.zone" class="badge ml-2">{{ iface.zone }}</span>
        <span v-if="iface.description" class="ml-2 text-sm text-neutral-500">
          {{ iface.description }}
        </span>
      </h2>

      <p v-if="!iface.present" class="text-sm text-amber-700 dark:text-amber-400">
        This link is not up yet, so nothing is queued on it.
      </p>

      <div class="grid gap-6 sm:grid-cols-2">
        <div v-for="d in directions(iface)" :key="d.name" class="space-y-2">
          <div class="text-neutral-500">{{ d.label }}</div>
          <div class="text-2xl leading-tight tabular-nums">
            ...
            <span class="text-base text-neutral-500">of {{ formatRate(d.state.rate) }}</span>
          </div>
          <div
            class="meter bg-sky-100 dark:bg-sky-500/20"
            role="meter"
          ...
          >
            <div
              class="meter-fill bg-sky-600 dark:bg-sky-500"
              :style="{ width: `${fill(`${iface.name}:${d.name}`, d.state.rate)}%` }"
            ></div>
          </div>

          <p v-if="!d.state.installed" class="text-sm text-neutral-500">Not in the kernel yet.</p>
