...
describe('UpdateModeFields', () => {
  ...
  // Disabled means nothing happens on its own, so neither schedule has
  // anything to say.
  it('drops both rows on disabled and says what is left', () => {
    const w = open({ mode: 'disabled' })
    expect(w.find('#upd-check-schedule').exists()).toBe(false)
    expect(w.find('#upd-install-schedule').exists()).toBe(false)
    expect(w.text()).toContain('Check now still works')
  })
