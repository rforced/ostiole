...
import AccessSection from '@/views/system/AccessSection.vue'
...
vi.mock('@/lib/api', () => ({
  ...
  api: {
    ...
    tokens: { list: vi.fn(), create: vi.fn(), remove: vi.fn() },
    auth: { me: vi.fn() },
  },
}))
...
async function open(accounts = [account('admin', 'admin'), account('watcher', 'viewer')]) {
  ...
  const wrapper = mount(AccessSection, { global: { stubs: { AppDialog: AppDialogStub } } })
  await flushPromises()
  return wrapper
}
...
describe('AccessSection accounts', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('creates an account and redraws from what the server returned', async () => {
    ...
    await button(wrapper, 'New account').trigger('click')
