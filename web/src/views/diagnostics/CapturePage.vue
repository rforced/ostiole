...
<template>
  <div class="space-y-4">
    ...
    <form class="form-row" @submit.prevent="capture.run()">
      ...
      <FormField id="cp-addr" label="Address" hint="Matches source or destination. Empty for all.">
    ...
    <p v-if="capture.error.value" role="alert" class="text-sm text-red-600 dark:text-red-400">
      {{ capture.error.value }}
    </p>
  </div>
</template>
