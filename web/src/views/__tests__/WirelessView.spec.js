...
describe('WirelessStatus', () => {
  ...
  // "stopped" on its own sent people to the journal. hostapd says why it
  // gave up just before it does, and that line belongs on the card.
  it('says why a radio that should be on air is stopped', async () => {
    const draft = config()
    draft.interfaces.push({
      name: 'ap0',
      enabled: true,
    ...
    const stopped = { reason: 'ap0: Could not set channel for kernel driver' }
    const wrapper = await strip({ setUp: true, country: 'US', radios: [card({ stopped })] }, draft)
    expect(wrapper.text()).toContain('stopped')
    expect(wrapper.text()).toContain('Could not set channel for kernel driver')
  })
})
