...
export const useConfigStore = defineStore('config', () => {
  ...
  const upsertProxyRoute = (route, previousId) => upsertIn('routes', route, previousId)
  ...
  const removeProxyRoute = (id) => removeFrom('routes', id, `Deleted route ${id}.`)

  /** Switches a rule off for a profile, from the events tab. */
  function addExclusion(profileId, exclusion) {
    const profile = (proxy.value.wafProfiles ?? []).find((w) => w.id === profileId)
    if (!profile) return
    const already = (profile.exclusions ?? []).some(
      (e) => e.rule === exclusion.rule && (e.path ?? '') === (exclusion.path ?? ''),
    )
    if (already) return
    const where = exclusion.path ? ` on ${exclusion.path}` : ''
    undoable(`Excluded rule ${exclusion.rule}${where} in the draft.`, () => {
      const w = (ensureProxy().wafProfiles ?? []).find((x) => x.id === profileId)
  ...
  const dnsProviders = computed(() => draft.value?.acme?.providers ?? [])
  ...
  function upsertDnsProvider(provider, was) {
    const acme = ensureACME()
    const list = acme.providers ?? (acme.providers = [])
  ...
  function removeDnsProvider(id) {
    undoable(`Deleted DNS provider ${id}.`, () => {
      ensureACME().providers = dnsProviders.value.filter((p) => p.id !== id)
    })
  }

  /** The certificates whose challenge this provider answers. */
  function providerDependents(id) {
    return certificates.value.filter((c) => c.provider === id).map((c) => c.id)
  }
