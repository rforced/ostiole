...
test('subscribe to a blocklist, fetch it, and ask why a name is blocked', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'DNS', level: 1 })).toBeVisible()

  await page.getByRole('tab', { name: 'Blocking' }).click()
  await page.getByLabel('Block lists enabled').check()
...
test('turning the lists off leaves enforcement in force and says why a refresh fetched nothing', async ({
  ...
  await login(page)
  await page.goto('/services/dns#blocking')
  await page.getByLabel('Block lists enabled').uncheck()
  await applyAndConfirm(page)
